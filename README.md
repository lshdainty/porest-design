...
npm run verify            # sync 검사 + lint:all (머지 전 게이트)
...
| `npm run verify` | sync:check + lint:all (pre-commit hook이 자동 실행) |
...
5. **검증**: `npm run verify` (sync:check + lint:all). 통과해야 commit 가능 (pre-commit hook이 자동 실행).
...
- `npm ci` → `npm run verify` (sync:check + lint:all)
