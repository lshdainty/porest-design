...
- 브랜드 accent 호환 (버튼/아이콘 1차 용도 — UI 3:1 기준)
  - `primary-hr` on `surface-default` **5.16:1** / on `surface-input` **4.61:1** — 본문도 통과
  - `primary-desk` on `surface-default` **5.02:1** / on `surface-input` **4.48:1** — UI 3:1 ✅, 본문은 4.5에 0.02 못 미침. accent는 본문 텍스트 색상으로 사용 금지(버튼 fill·아이콘·강조 텍스트 한정)로 운용.
...
| text-on-accent `#FFFFFF` | primary-hr | 5.16 | AA |
| text-on-accent | primary-desk | 5.02 | AA |
...
3. `border-focus`는 이번 배치에서 보류 — 브랜드 accent가 `surface-default-dark`에서 3:1 미달(primary-hr **2.77:1**, primary-desk **2.85:1**)이므로 `accent-*-light` 변형이 정의된 후 추가하는 편이 안전. 그 전까지 포커스 링은 `border-strong`을 임시 활용 가능(모든 표면에서 3:1+).
...
| 토큰 | hex | 사용 |
|---|---|---|
| `primary-hr-light` | `#3FBF74` | HR — 어두운 표면(다크 모드 + 라이트 모드 검정 배너 등) 위 텍스트·아이콘·outlined 버튼·focus 링 |
| `primary-desk-light` | `#6DA8F2` | Desk — 어두운 표면(다크 모드 + 라이트 모드 검정 배너 등) 위 텍스트·아이콘·outlined 버튼·focus 링 |

#### 추가 이유
1. v3에서 `border-focus`가 보류된 직접 원인 해소 — `primary-hr` 2.77:1, `primary-desk` 2.85:1로 `surface-default-dark` 대비 3:1 미달. 다크 lightness 변형으로 4.5:1 이상 확보.
...
| 텍스트 | 다크 표면 | 대비 | 결과 |
|---|---|---|---|
| primary-hr-light `#3FBF74` (L=0.396) | surface-default-dark | 6.07 | ✅ AAA |
| primary-hr-light | bg-page-dark | 6.89 | ✅ AAA |
| primary-hr-light | surface-input-dark | 5.23 | ✅ AA |
| primary-desk-light `#6DA8F2` (L=0.376) | surface-default-dark | 5.81 | ✅ AAA |
| primary-desk-light | bg-page-dark | 6.59 | ✅ AAA |
| primary-desk-light | surface-input-dark | 5.00 | ✅ AA |

#### 채움 fill 비호환 — 사용 경계 명시

`accent-*-light` 위에 `text-on-accent` (`#FFFFFF`) 사용 시 본문 대비:
- on `primary-hr-light`: **2.36:1** ❌
- on `primary-desk-light`: **2.46:1** ❌

따라서 **다크 모드 채움 버튼 fill은 `primary-hr`/`primary-desk`(원래 값)를 유지**합니다. 흰 텍스트 5.16/5.02:1로 본문 통과. 단, 이 경우 버튼 외곽 vs 다크 표면 대비는 2.77/2.85:1로 3:1 미달 — Toss·Material 패턴처럼 **inset shadow 또는 명시적 1px 외곽선**(예: `border-strong-dark`)으로 컴포넌트 식별 보강 필요(컴포넌트 스펙에서 처리).

요약하면:
- **다크 채움 버튼**: bg = `primary-hr` / `primary-desk`, text = `text-on-accent`(`#FFFFFF`), 외곽선 = `border-strong-dark` 보강
- **어두운 표면 비채움 사용**(outlined 버튼 텍스트·링크·아이콘·focus, 라이트/다크 모드 무관): `primary-hr-light` / `primary-desk-light`
...
| 토큰 | hex | 시맨틱 |
|---|---|---|
| `success` | `#117A3A` | 완료·확인·긍정 결과 (forest green — primary-hr emerald와 hue 분리) |
| `error` | `#C53030` | 오류·파괴적 액션·필수 입력 누락 |
| `warning` | `#A85800` | 경고·주의·임박 만료 (deep amber) |
| `info` | `#006395` | 안내·도움말·진행 중 (deep navy — primary-desk vibrant blue와 채도·명도 분리) |
...
- 시각 차별화: `success`(forest)는 `primary-hr`(emerald)와 미세 hue 분리, `info`(deep navy)는 `primary-desk`(vibrant)와 채도 분리. 단 단독 노출 시 식별성을 위해 컴포넌트 레벨에서 아이콘(✓/✕/!/i) 동반을 권장(prose 가이드 영역).
...
| 작업 | 이전 | 이후 | 비고 |
|---|---|---|---|
| rename + value | `accent-hr` `#1E7D4C` | `primary-hr` `#357B5F` | HR 브랜드 — forest green |
| rename + value | `accent-desk` `#1B6ADB` | `primary-desk` `#0147AD` | Desk 브랜드 — deep blue |
| rename + value | `accent-hr-light` `#3FBF74` | `primary-hr-light` `#6BAE8C` | 새 base에 맞춰 lighten 도출 |
| rename + value | `accent-desk-light` `#6DA8F2` | `primary-desk-light` `#6BA0EE` | 새 base에 맞춰 lighten 도출 |
...
|---|---|---|---|
| `button-primary-hr` | primary-hr `#357B5F` | text-on-accent | ✅ ≥4.5:1 |
| `button-primary-desk` | primary-desk `#0147AD` | text-on-accent | ✅ ≥4.5:1 |
| `button-outlined-hr-on-dark` | surface-default-dark | primary-hr-light `#6BAE8C` | ✅ ≥4.5:1 |
| `button-outlined-desk-on-dark` | surface-default-dark | primary-desk-light `#6BA0EE` | ✅ ≥4.5:1 |
...
- **`primary-hr` as inline 텍스트 on `bg-page-hr`**: 손계산 **4.14:1** (4.5:1 미달, 3:1 통과)
  - **운영 규칙**: HR `primary-hr`는 **fill 전용**(button bg, badge, fill icon). 페이지 베이지 위 inline 링크·body 텍스트로 사용 금지 — 본문 가독성 미달.
  - 대신 inline 링크 needs use `text-primary` (대비 13.32:1) + 밑줄/hover 시그널로 link affordance 확보.
- **`primary-desk` as inline on `bg-page-desk`**: 손계산 **6.13:1** ✅ — Desk primary는 inline 사용 가능.
...
- HR 다크 채움 버튼: 새 `primary-hr` `#357B5F` (L=0.158) on white text **5.05:1** ✅. 단 외곽 vs 다크 surface는 2.83:1로 3:1 미달 — `border-strong-dark` 보강 패턴 그대로 유효(이전 `accent-hr` 2.77과 동급).
...
| 토큰 | hex (mirror) | 용도 |
|---|---|---|
| `border-focus-hr` | `#357B5F` (= primary-hr) | HR 라이트 표면 위 focus ring |
| `border-focus-hr-light` | `#6BAE8C` (= primary-hr-light) | HR 다크 표면 위 focus ring |
| `border-focus-desk` | `#0147AD` (= primary-desk) | Desk 라이트 표면 위 focus ring |
| `border-focus-desk-light` | `#6BA0EE` (= primary-desk-light) | Desk 다크 표면 위 focus ring |
...
|---|---|---|---|
| `border-focus-hr` `#357B5F` | surface-default | 5.05 | ✅ |
| `border-focus-hr` | surface-input | 4.51 | ✅ |
| `border-focus-hr` | bg-page | 4.67 | ✅ |
| `border-focus-hr-light` `#6BAE8C` | surface-default-dark | 5.49 | ✅ |
| `border-focus-hr-light` | surface-input-dark | 4.72 | ✅ |
| `border-focus-hr-light` | bg-page-dark | 6.23 | ✅ |
| `border-focus-desk` `#0147AD` | surface-default | 8.40 | ✅ |
| `border-focus-desk` | surface-input | 7.50 | ✅ |
| `border-focus-desk` | bg-page | 7.78 | ✅ |
| `border-focus-desk-light` `#6BA0EE` | surface-default-dark | 5.37 | ✅ |
| `border-focus-desk-light` | surface-input-dark | 4.62 | ✅ |
| `border-focus-desk-light` | bg-page-dark | 6.09 | ✅ |
...
- **다크 모드 shadow**: rgba 흑색 shadow는 다크 표면 위에서 거의 안 보임 — 다크 모드에서는 **shadow 효과를 약화하고 `border-default-dark` 외곽선 + 표면 휘도 차로 elevation 표현**(컴포넌트 스펙에서 처리). 다크 모드 전용 `shadow-*-dark` 변형은 도입 보류 — light shadow on dark의 직관적 디자인 솔루션이 정착되지 않음.
