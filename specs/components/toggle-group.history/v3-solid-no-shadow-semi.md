...
| `solid` | `primary` 채움 + `text-on-accent` + font-bold (700) + `0 1px 3px rgba(0,0,0,0.15)` subtle shadow — cobalt brand 강조 | transparent + `text-secondary` | form type 선택, 모드 전환 같이 ‟현재 선택‟ 을 명확히 부각해야 할 때. |
...
| `on` (`data-state=on`) | `primary` | `text-on-accent` + 700 | subtle shadow `0 1px 3px rgba(0,0,0,0.15)` |
| `on + hover` | `primary` | `text-on-accent` + 700 | (동일) |
| `disabled` | (동일) opacity 0.5 | (동일) | `pointer-events: none` |
