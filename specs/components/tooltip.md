...
Porest Tooltip은 **단일 spec × 4 sides(top/right/bottom/left)** 매트릭스로 정의됩니다. **surface-tone tooltip** — light 모드는 흰색(`surface-default` #FFFFFF) + `text-primary` 검정 텍스트, dark 모드는 `surface-default-dark` 자동 swap + `text-primary-dark` 흰색 텍스트. border 1px(`border-default`)로 페이지 배경과 분리하고 가벼운 `shadow-sm`으로 절제된 elevation 표현. modal/popover와 동일한 surface 톤을 유지해 시각 일관성 우선.
...
```
   ...
   └──────┬───────┘
          ...
          ▼  (sideOffset 4)
   ┌──────────────┐
   │ 도움말 텍스트 │   ← bg `surface-default`(swap), text `text-primary`(swap)
   └──────────────┘   ← border `border-default` + shadow-sm 가볍게
       caption / label-sm
```

| ⓐ content | `z-50 overflow-hidden rounded-xs border border-border-default px-[var(--spacing-md)] py-[var(--spacing-xs)] text-label-sm` + `style={{ backgroundColor: "var(--color-surface-default)", color: "var(--color-text-primary)", boxShadow: "var(--shadow-sm)" }}` |
...
- 배경/텍스트 색은 **`background-color`/`color` inline style 직접 인용** — `var(--color-surface-default)` + `var(--color-text-primary)`는 light/dark 자동 swap되어 두 테마 모두 자연스러운 surface 톤 유지.
- box-shadow도 inline style — Tailwind v4 `shadow-sm` utility의 `--tw-shadow-*` 분해 처리 우회 (Card/Dialog/Drawer/Popover/Sonner와 동일 패턴).
- **border 1px 필수** — light 모드에서 `surface-default`(#FFFFFF)와 `bg-page`(#F5F6FA) 대비 1.05:1 약함. border가 시각 식별 보강 + shadow-sm은 가벼운 elevation.
...
| Background | `surface-default` (light:#FFFFFF / dark:#242938) | `var(--color-surface-default)` |
| Text | `text-primary` (light:#1A1F2E / dark:#F5F6FA) | `var(--color-text-primary)` |
| Border | 1px `border-default` | `border border-border-default` |
...
| **WCAG 1.4.3** Color contrast (light: `text-primary` × `surface-default`) | 14:1+ ✓ (AAA 본문 7:1 초과) |
| **WCAG 1.4.3** Color contrast (dark: `text-primary-dark` #F5F6FA × `surface-default-dark` #242938) | 12:1+ ✓ |
| **WCAG 1.4.11** Non-text contrast (light: surface × bg-page) | #FFFFFF × #F5F6FA = 1.05:1 — 단독 약함이나 **border 1px `border-default`** + shadow-sm가 시각 식별 보강 ✓ |
| **WCAG 1.4.11** Non-text contrast (dark: surface × bg-page) | #242938 × #1A1F2E = 1.4:1 — 다크 모드도 border + shadow-sm로 보강 |
...
- border 없이 사용 — light 모드에서 surface-default와 bg-page가 거의 흰색끼리라 분리 안 보임. border 1px 필수.
- shadow-md/lg 같은 강한 그림자 — tooltip은 부가 hint라 절제 톤. shadow-sm 권장.

## Migration notes

- 기존 `tooltip.tsx`는 `bg-text-primary` Tailwind utility 사용 — 다크 모드에서 `--color-text-primary`가 `--color-text-primary-dark`(#F5F6FA)로 swap되어 tooltip 배경이 white-ish가 되고, 흰색 텍스트(`text-on-accent`)와 invisible 충돌. **surface-default + text-primary 패턴**으로 정정 — `--color-surface-default`는 light(#FFFFFF) / dark(#242938) 자동 swap, `--color-text-primary`도 함께 swap되어 두 모드 모두 자연스러운 surface 톤.
- 초기 spec에서 Toss/Apple 스타일 inverted dark(stable dark bg + white text)를 검토했으나 사용자 피드백("light 모드도 흰색")으로 **surface-tone 패턴 채택**. 페이지 일관성 우선.
- light 모드 surface-default(#FFFFFF) × bg-page(#F5F6FA) 대비 1.05:1로 단독 약함 → **border 1px `border-default` 필수** + shadow-sm로 시각 식별 보강. dark 모드도 border + shadow 둘 다 유지.
