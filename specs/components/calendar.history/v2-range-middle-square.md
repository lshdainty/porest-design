...
| ⓐ root (`DayPicker`) | `p-[var(--spacing-md)] [--cell-size:40px]` — 외곽 padding + day cell 크기 CSS var. Card/Popover 안에 wrap 권장. |
| ⓑ month_caption + button_previous/next | `month_caption`: `flex items-center justify-center h-[var(--cell-size)] text-title-sm font-medium text-text-primary`. `button_previous` / `button_next`: [`Button`](button.md) `variant="outline"` size 28×28 + `Chevron` component(`orientation="left|right"`) 16. |
| ⓒ weekdays + weekday | `weekdays`: `<tr>` — `flex`. `weekday`: `<th>` — `text-caption font-semibold uppercase tracking-wide text-text-tertiary flex-1 select-none` + 한글 요일("일/월/화/수/목/금/토") via `react-day-picker` `locale={ko}`. 토/일은 weekend 톤(`text-tertiary` 유지 또는 별도 시각). |
| ⓓ week (`week`) | `<tr>` — `flex w-full mt-[var(--spacing-xs)]`. 한 주(7 day cell). |
| ⓔ day (`day`) | `<td>` — `relative w-full h-full p-0 text-center text-body-md aspect-square select-none`. range mode에서 first/last selected 셀에 rounded 양 끝. |
| ⓕ DayButton (`day_button`) | `<button>` (custom `components.DayButton`) — `inline-flex aspect-square w-full min-w-(--cell-size) items-center justify-center rounded-full p-0 text-body-md font-normal text-text-primary hover:bg-surface-input focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring transition-colors duration-[var(--motion-duration-fast)]`. v9는 day button focus 관리를 `DayButton` 컴포넌트로 분리(modifiers.focused). |
| ⓖ selected (modifier) | `data-[selected-single=true]:bg-primary data-[selected-single=true]:text-text-on-accent data-[selected-single=true]:rounded-full` — 선택 fill. |
| ⓗ today (`today`) | `outline outline-2 outline-primary outline-offset-[-2px] font-semibold rounded-full data-[selected=true]:outline-0` — **테두리만**(fill 없음, 선택 가능 상태 유지). preview `.cal-cell--today` 정합. |
| ⓘ outside (`outside`) | `text-text-tertiary` — 이전/다음 달 날짜(시각 약화). |
| ⓙ disabled (`disabled`) | `text-text-tertiary opacity-50 cursor-not-allowed` — 휴일/제한 날짜. |
| ⓚ range_start / range_end | 양 끝 fill(`data-[range-start=true]:bg-primary data-[range-end=true]:bg-primary`) + rounded-full. |
| ⓛ range_middle | `data-[range-middle=true]:bg-[color-mix(in_srgb,var(--color-primary)_12%,transparent)] data-[range-middle=true]:text-text-primary data-[range-middle=true]:rounded-none` (시작/끝 사이 연속 채움). |
...
| v8 (옛) | v9 (현재) |
|---|---|
| `caption` (root flex) | `month_caption` |
| `caption_label` | `caption_label` (유지) |
| `nav_button` + `nav_button_previous` / `nav_button_next` (absolute) | `button_previous` / `button_next` (자동 배치) |
| `head_row` | `weekdays` |
| `head_cell` | `weekday` |
| `row` | `week` |
| `cell` | `day` (자체 `<td>`) |
| `day` | `DayButton` (custom `components.DayButton`, focus 관리 자동) |
| `day_selected` | `selected` (modifier, `data-[selected-single=true]` 등) |
| `day_today` | `today` |
| `day_outside` | `outside` |
| `day_disabled` | `disabled` |
| `day_range_start` / `_end` / `_middle` | `range_start` / `range_end` / `range_middle` |
| `day_hidden` | `hidden` |
| `IconLeft` / `IconRight` (components slot) | `Chevron` (single slot, `orientation="left|right|down"` prop) |
| (없음) | `getDefaultClassNames()` — 기본 className 인용 (cn 합성) |
| (없음) | `--cell-size` CSS var — day cell 크기 한 곳에서 조정 |
| (없음) | `captionLayout` prop — `"label"`(default) / `"dropdown"`(연·월 드롭다운) |

### Porest 시각 토큰 정정 (spec과 정합)

- day cell: `h-10 w-10 rounded-full text-body-md` (preview `.cal-cell` SoT, Toss 톤 원형 + body 가독성).
- today: `outline outline-2 outline-primary outline-offset-[-2px] font-semibold` (preview `.cal-cell--today` 정합 — fill 대신 outline only, selected와 시각 분리).
- selected (single): `data-[selected-single=true]:bg-primary` + `rounded-full` (DayButton에서 적용).
- weekday: `text-caption font-semibold uppercase tracking-wide text-text-tertiary flex-1`.
- month_caption: `text-title-sm font-medium text-text-primary`.
- transition: `transition-colors duration-[var(--motion-duration-fast)] ease-[var(--motion-ease-out)]`.

### 한글 locale

`import { ko } from "date-fns/locale"` + `<Calendar locale={ko} />`. v9도 date-fns locale 그대로 사용 — 요일/월 한글 자동.

### 추가 노트

- examples 한국 도메인 정합 — single(Desk 가계부) / range(HR 휴가) / multiple modifiers(HR 근태 표시) 3종.
- preview-html `.cal-*` CSS는 className 별도 (v8/v9 무관) — 이미 시각 정합. 변경 없음.
- range middle 시각: spec은 `bg-surface-input` 통합 톤이었으나 v9 마이그레이션 시 `color-mix(primary 12%)`로 진화(preview의 Toss 톤 정합).
