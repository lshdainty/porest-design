...
| ⓒ TableHeader | `<thead>` — `bg-bg-page [&_tr]:border-b border-border-default`. 페이지 바탕색으로 thead 영역을 row(`bg-surface-default`)와 미세 분리. |
| ⓓ TableHead | `<th>` — `h-10 px-[var(--spacing-sm)] text-left align-middle text-caption font-semibold uppercase tracking-wide text-text-tertiary`. data table 톤 — 헤더가 데이터 위계 보조라 더 작고 약하게(text-caption 12 + tertiary). sortable column은 [`Button`](button.md) `variant="ghost" size="sm"` wrap + 화살표(↑/↓) 16 icon. active sort는 `text-text-primary` + 화살표 노출. |
...
- 기존 `table.tsx` 정정:
  - Table `text-title-sm` (16/500) → **`text-body-md`** (15/400) — data 가독성 우선, title-sm은 본문 헤더용.
  ...
  - **TableHeader/TableHead — preview `.dt-*` SoT 정합 (v17 정정)**:
    - TableHeader `bg-bg-page` 추가 — thead 영역을 row(`surface-default`)와 미세 분리.
    - TableHead `text-body-md font-medium text-text-secondary` → `text-caption font-semibold uppercase tracking-wide text-text-tertiary` — data table 헤더 톤(헤더가 데이터 위계 보조라 작고 약하게).
    - 이유: spec 초안에서 일반 본문 헤더(body-md + medium + secondary)로 갔으나 preview `.dt-*` SoT(`bg-page` + 11px + uppercase + tertiary)와 어긋남. 사용자 피드백 — preview 톤이 data table에 더 자연. preview 정합으로 일관성.
- examples 한국 도메인 재작성:
