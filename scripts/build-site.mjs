...
function buildCombinedTokens() {
  // Tailwind v4 CDN(@tailwindcss/browser)이 @theme {} 블록을 읽어 utility class 자동 생성.
  // brand override는 [data-brand="..."] 셀렉터로 var(--color-primary) 값만 변경 →
  // .bg-primary 같은 utility class가 자동으로 brand 색상 따름.
  // FOUC 방지 — @theme 와 동일 내용을 :root 에도 emit (브라우저 즉시 적용 + Tailwind 처리 둘 다 가능).
  ...
  // Default tokens.css는 이미 @theme {} 형태 — 그대로 사용. 단 baseline에 brand fallback 추가.
  ...
  // @theme 내용을 추출해 :root 블록으로도 emit (FOUC 방지)
  const themeMatch = /@theme\s*\{([\s\S]*?)\n\}/.exec(defaultWithFallback);
  const rootBlock = themeMatch ? `:root, [data-brand="default"] {${themeMatch[1]}\n}` : "";

  // brand override — [data-brand="..."] 셀렉터로 CSS variable 재정의
  ...
  return [
    ...
    "/* @theme {} block — Tailwind v4 CDN reads this for utility class generation */",
    "/* :root 블록은 FOUC 방지용 — 브라우저가 @theme 처리 전에도 변수 즉시 적용 */",
    "/* HR / Desk brand override = [data-brand=\"...\"] 셀렉터로 CSS variable 재정의 */",
    "",
    defaultWithFallback,
    "",
    "/* Browser fallback — :root에 동일 변수 직접 emit (Tailwind CDN 로드 전에도 적용) */",
    rootBlock,
    "",
    "/* Brand overrides — @theme 외부에서 CSS variable만 재정의 (utility는 default 정의 재사용) */",
    brandOverride(hr, "hr"),
    "",
